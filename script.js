...
const MISSING_COUNT = {EASY: 2, NORMAL: 3, HARD: 4};
const ROUND_COUNT = 3;
const SCORE_RULE = {CORRECT: 100, WRONG: -10};
const TIME_LIMIT = {EASY: 60, NORMAL: 45, HARD: 30};
...
const render = {
    initDigits: (texts) => {
        let allTexts = texts.concat(Array(ALL_DIGITS.length - texts.length).fill(''));
...
function init() {
    ...
    dom.digits.forEach(digit => {
        digit.addEventListener('touchstart', touchKey);
    });
...
async function startGame() {
    ...
    answerCount = MISSING_COUNT[dom.level().value.toUpperCase()];
    timer.setLimit(TIME_LIMIT[dom.level().value.toUpperCase()]);
    newRound();
    timer.start();
    canPress = true;
}
...
function touchKey(e) {
    ...
    if (!ALL_DIGITS.includes(digit)) return;

    handleDigitPress(digit);
}

function handleDigitPress(digit) {
    let foundDigit = _.find(digits, x => (x.text === digit));
    if (foundDigit.isPressed) return;

    foundDigit.isPressed = true;
    render.updateDigitStatus(foundDigit.text, foundDigit.isAnwser);

    score += foundDigit.isAnwser ? SCORE_RULE.CORRECT : SCORE_RULE.WRONG;
...
function Timer(render) {
    ...
    this.tickTock = () => {
        ...
        const totalTime = this.time.minute * 60 + this.time.second;
        if (totalTime >= this.limit) {
